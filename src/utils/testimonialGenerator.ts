...
import { baseTestimonials, genericNames, genericLastNames, genericTexts } from '../data/testimonials';
...
const getRandomRating = (): number => {
  ...
  if (rand < 70) { // 70% chance for 5 stars
    ...
    return 4;
  } else if (rand < 70 + 25 + 3) { // 3% chance for 3 stars
    return 3;
...
const formatNameInitialLastName = (fullName: string): string => {
  if (!fullName) return '';
...
export const generateTestimonials = (count: number): Testimonial[] => {
  ...
  const generated: Testimonial[] = Array.from({ length: count - processedBaseTestimonials.length }, (_, i) => {
  ...
  return [...processedBaseTestimonials, ...generated];
};