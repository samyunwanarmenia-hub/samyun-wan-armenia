import { TranslationKeys } from '../../types/global';

export const ru: TranslationKeys = {
  nav: {
    home: 'Главная',
    about: 'О нас',
    benefits: 'Преимущества',
    testimonials: 'Отзывы',
    contact: 'Контакты',
    faq: 'Часто задаваемые вопросы',
    products: 'Продукты',
    trackOrder: 'Отследить заказ', // New translation
    armenian: 'Армянский',
    russian: 'Русский',
    english: 'Английский',
    open: 'Открыть меню',
    close: 'Закрыть меню',
  },
  seo: {
    metaTitle: 'Samyun Wan — что это и что скрывают продавцы',
    metaDescription:
      'Перед покупкой Samyun Wan изучите реальный состав, эффект и факты, которые скрывают продавцы, чтобы выбрать только безопасный вариант.',
    metaKeywords: 'Samyun Wan, Самьюн Ван, набор веса, натуральная добавка, разоблачение подделок',
  },
  hero: {
    title: 'Samyun Wan Armenia',
    facebookFollow: 'Следите за нашими обновлениями в Facebook',
    subtitle: 'ОРИГИНАЛЬНЫЕ и ЭФФЕКТИВНЫЕ средства для регулирования веса',
    tagline: 'Изменив свое телосложение, изменится и твоя жизнь',
    cta: 'Заказать сейчас',
    consultation: 'Бесплатная консультация',
    guarantee: '100% гарантия подлинности',
    qrVerificationTitle: 'Сканируйте QR-код',
    qrVerificationSubtitle: 'Сканируйте для проверки',
    seo_title_addon: 'Что такое Samyun Wan и его реальный эффект | Спортивное питание',
    qrBlockTitle: 'Проверьте подлинность',
    qrBlockDescription: 'Отсканируйте QR-код, чтобы проверить подлинность продукта.',
  },
  stats: {
    customers: 'Довольных клиентов',
    experience: 'Лет опыта',
  },
  about: {
    title: 'Почему выбирают Samyun Wan Armenia',
    subtitle: 'Единственный официальный представитель в Армении',
    description: 'Samyun Wan Armenia является официальным представителем Samyun Wan в Армении. Мы предлагаем только оригинальную и качественную продукцию для здорового набора веса и общего благополучия.',
    content: 'Наша цель — предоставить вам лучшие натуральные решения для улучшения вашего здоровья и телосложения. Мы верим, что натуральные ингредиенты могут творить чудеса, и наша продукция — яркое тому подтверждение.',
    natural: {
      title: 'Натуральные ингредиенты',
      desc: '100% натуральные травы без химических добавок'
    },
    proven: {
      title: 'Доказанная эффективность',
      desc: 'Тысячи довольных клиентов по всему миру'
    },
    safe: {
      title: 'Безопасное использование',
      desc: 'Минимальные побочные эффекты, медицинский контроль'
    },
    fast: {
      title: 'Быстрые результаты',
      desc: 'Первые результаты через 1-2 недели'
    },
    whyChooseUsTitle: 'Почему выбирают нас',
    whyChooseUsOriginal: 'Оригинальный продукт – Мы являемся официальным представителем Samyun Wan в Армении уже 6 лет.',
    whyChooseUsSafety: 'Безопасность – Каждая упаковка имеет QR-код, который позволяет проверить подлинность.',
    whyChooseUsNoAdditives: 'Нет поддельных добавок – Только оригинальный продукт, никогда не содержит лекарств или вредных веществ.',
    whyChooseUsTrust: 'Доверие клиентов – Наши клиенты получают безопасный и подлинный продукт.',
  },
  benefits: {
    title: 'Преимущества Samyun Wan',
    subtitle: 'Доказанные результаты тысячами клиентов',
    appetite: {
      title: 'Повышение аппетита',
      desc: 'Естественно повышает аппетит и улучшает самочувствие'
    },
    weight: {
      title: 'Набор веса',
      desc: 'Здоровым способом увеличивает мышечную массу и общий вес'
    },
    immunity: {
      title: 'Иммунитет',
      desc: 'Укрепляет иммунную систему и улучшает пищеварение'
    },
    energy: {
      title: 'Энергия',
      desc: 'Повышает уровень энергии и снижает усталость'
    },
    metabolism: {
      title: 'Метаболизм',
      desc: 'Улучшает метаболизм и усвоение питательных веществ'
    },
    mood: {
      title: 'Настроение',
      desc: 'Улучшает настроение и общее качество жизни'
    },
  },
  testimonials: {
    title: 'Отзывы',
    subtitle: '',
    formTitle: 'Оставьте свой отзыв',
    formSubtitle: 'Мы будем рады услышать вашу историю',
    namePlaceholder: 'Ваше имя',
    reviewPlaceholder: 'Ваш отзыв',
    submitButton: 'Отправить отзыв',
    thankYou: 'Спасибо за ваш отзыв. Ваше мнение очень важно для нас и для других.',
    newReviewLabel: 'Новый отзыв'
  },
  contact: {
    title: 'Контакты',
    address: 'Адрес',
    phone: 'Телефон',
    hours: 'Рабочие часы',
    phoneNumbers: {
      number1: '+374 96 653666',
      number2: '+374 95 653666',
      description: 'Свяжитесь с нами по телефону или WhatsApp'
    },
    callNowButton: 'Позвонить сейчас',
    whatsappButton: 'WhatsApp'
  },
  footer: {
    about: 'О нас',
    products: 'Продукты',
    support: 'Поддержка',
    follow: 'Подпишитесь на нас',
    legalAndWarning: `<section class="footer-warning" aria-label="Предупреждение">
      <div class="footer-warning__grid footer-warning__grid--balanced">
        <article class="footer-warning__card footer-warning__card--lead">
          <p class="footer-warning__title">Предупреждение</p>
          <p class="footer-warning__subtitle">Это не лекарство</p>
          <p class="footer-warning__text">Не применяйте препарат самостоятельно и не заменяйте им назначенное лечение.</p>
          <ul class="footer-warning__list">
            <li>беременным женщинам</li>
            <li>кормящим матерям</li>
            <li>детям младше 12 лет</li>
          </ul>
        </article>
        <article class="footer-warning__card footer-warning__card--info">
          <p class="footer-warning__heading">О нас</p>
          <p class="footer-warning__text">Наша команда — официальный представитель бренда в Армении.</p>
          <ul class="footer-warning__details">
            <li>Официальная доставка по всей Армении</li>
            <li>Персональные программы и сопровождение</li>
            <li>Поддержка в Telegram и WhatsApp</li>
          </ul>
          <a class="footer-warning__action" href="#about">Подробнее</a>
        </article>
        <article class="footer-warning__card footer-warning__card--products">
          <p class="footer-warning__heading">Продукция</p>
          <p class="footer-warning__text">Поставляем только оригинальные наборы Samyun Wan с QR-проверкой.</p>
          <ul class="footer-warning__details">
            <li>Комплекты и комплексные программы</li>
            <li>Лимитированные серии</li>
            <li>Прозрачное происхождение и документы</li>
          </ul>
          <a class="footer-warning__action" href="#products">Смотреть каталог</a>
        </article>
      </div>
      <p class="footer-warning__note">Используйте только каналы связи, указанные на сайте, и заранее проверяйте подлинность товара.</p>
    </section>`,
    howToDistinguishButton: 'Смотреть здесь',
    productOriginal: 'Samyun Wan Оригинал',
    productAuthenticity: 'Проверка подлинности',
    productUsage: 'Руководство по использованию',
    allRightsReserved: 'Все права защищены.',
    caution: 'Пользуйтесь только контактами, указанными на этом сайте, и остерегайтесь поддельных страниц.',
  },
  contactModal: {
    chooseCall: 'Выберите номер для звонка',
    chooseMessage: 'Выберите способ связи',
    call: 'Звонок',
    whatsapp: 'WhatsApp',
    facebookMessenger: 'Facebook Messenger',
    callNumbers: {
      number1: '+374 96 653666',
      number2: '+374 95 653666'
    },
    callUsButton: 'Позвонить нам',
    callbackButton: 'Запросить обратный звонок',
  },
  orderModal: {
    title: 'Заказать Samyun Wan',
    deliveryInfo1: 'Для оформления заказа с доставкой в Ереване, пожалуйста, укажите адрес доставки и номер телефона.',
    deliveryInfo2: 'Хотим сообщить, что доставка осуществляется ежедневно с 16:00 до 22:00.',
    addressPlaceholder: 'Адрес доставки',
    phonePlaceholder: 'Номер телефона',
    orderButton: 'Заказать',
    orderSuccess1: 'Ваш заказ принят. Спасибо! 💚',
    orderSuccess2: 'До 22:00 курьер свяжется с вами заранее и доставит заказ.',
    invalidPhone: 'Пожалуйста, введите действительный номер телефона.',
    selectProducts: 'Выберите продукт(ы) для заказа:',
    freeDeliveryMessage: 'Бесплатная доставка по Еревану'
  },
  authenticity: {
    title: 'Отличите оригинал',
    howToDistinguish: 'Как отличить оригинал',
    attention: 'ВНИМАНИЕ',
    certificateDesc: 'Это сертификат подлинности Samyun Wan. Обратите внимание на логотип и защитные элементы.',
    differencesDesc: 'На этом изображении показано, как отличить оригинальный Samyun Wan от подделки. Следуйте инструкциям, чтобы избежать подделок.',
    waitingForLink: 'Пожалуйста, подождите, пока наш оператор отправит вам ссылку.',
    qrScanInstructions: 'Отсканируйте QR-код, чтобы проверить подлинность продукта.',
    qrScanSuccess: 'Запрос успешно отправлен.',
    qrScanError: 'Ошибка сканирования QR-кода.',
    processingRequest: 'Пожалуйста, подождите, ваш запрос обрабатывается.',
    recordingInstructions: 'Запишите короткое видео продукта для верификации.',
    recordingSuccess: 'Видео успешно отправлено для верификации.',
    recordingError: 'Ошибка записи или отправки видео.',
    purchaseWarning: 'Рекомендуем покупать только у надежных продавцов. Присоединяйтесь к тем, кто выбирает настоящий и 100% оригинальный Samyun Wan с QR-кодом и безопасностью.',
    disclaimer: 'Внимание: Продукт не является лекарственным средством и не предназначен для лиц младше 18 лет.',
    fakeWarning: '⚠️ ВНИМАНИЕ: Остерегайтесь подделок!',
    fakeWarningText: 'В интернете много мошенников, которые продают подделки! Покупайте только у официального дистрибьютора!',
    originalTitle: '✅ Оригинальный Samyun Wan',
    originalFeature1: '• QR-код на упаковке для проверки',
    originalFeature2: '• Покупается только у официального дистрибьютора',
    originalFeature3: '• Официальная упаковка с логотипом',
    originalFeature4: '• Сертификат подлинности',
    originalFeature5: '• Контакт: +37495653666',
    fakeTitle: '❌ Подделка',
    fakeFeature1: '• НЕТ QR-кода на упаковке',
    fakeFeature2: '• Продается на неизвестных сайтах',
    fakeFeature3: '• Поддельная упаковка',
    fakeFeature4: '• Нет сертификата',
    fakeFeature5: '• Опасна для здоровья!',
    verificationTitle: '🔍 Как проверить подлинность',
    verificationStep1: '1. Проверьте QR-код на упаковке',
    verificationStep2: '2. Покупайте только у официального дистрибьютора',
    verificationStep3: '3. Остерегайтесь неизвестных сайтов',
    verificationStep4: '4. Звоните +37495653666 для проверки',
    verifyButton: 'Проверить подлинность продукта',
  },
  article: {
    metaTitle: 'Samyun Wan реальные отзывы, подробное исследование, риски и надежный гид по покупке',
    metaDescription: 'Перед покупкой Samyun Wan изучите отзывы, состав, инструкции, противопоказания и признаки оригинала, чтобы не ошибиться с подделкой.',
    metaKeywords: 'Samyun Wan, Samyun Wan капсулы, Samyun Wan Армения, отзывы Samyun Wan, результаты Samyun Wan, побочные эффекты Samyun Wan, безопасные добавки для набора веса, стимуляторы аппетита, натуральный рост мышц, проверенные продавцы',
    title: 'Samyun Wan реальные отзывы, полное исследование, риски и доверенный гид по покупке',
    subtitle: 'Безопасность, состав и практический опыт перед покупкой',
    intro: 'Не полагайтесь только на рекламу: изучите реальные отзывы, ингредиенты, противопоказания и сигналы о подделках.',
    sections: [
      {
        id: 'capsules',
        heading: 'Самyun Wan капсулы. Что важно знать',
        description: 'Продаются на маркетплейсах по 5000 драмов, но почти всегда без сертификатов и регистрации.',
        bullets: [
          'Требуйте сертификат соответствия и QR-код с проверкой.',
          'Поддельные партии часто содержат неизвестные компоненты.',
          'Оригиналы подтверждаются документацией и QR-проверкой.',
        ],
      },
      {
        id: 'audience',
        heading: 'Кому нужен Samyun Wan',
        description: 'Добавка для тех, кто не может набрать вес из-за болезни или отсутствия аппетита.',
        bullets: [
          'Естественно худые люди',
          'Восстанавливающиеся после хронических заболеваний',
          'Те, у кого нет аппетита или нерегулярное питание',
          'Активные люди с быстрым обменом веществ',
        ],
      },
      {
        id: 'advantages',
        heading: 'Преимущества Samyun Wan',
        description: 'На основе трав, помогает улучшить усвоение калорий и увеличить аппетит без химии.',
        bullets: [
          'Натуральный состав без синтетики',
          'Повышает аппетит и частоту питания',
          'Быстрый набор массы с мягким воздействием',
          'Минимальные побочные эффекты при правильном применении',
        ],
      },
      {
        id: 'results',
        heading: 'Результаты по отзывам',
        description: 'Клиенты отмечают улучшение пищеварения, иммунитета и выносливости.',
        bullets: [
          'Лучшее пищеварение и усвоение нутриентов',
          'Сильный иммунитет и устойчивость к инфекциям',
          'Стабильные суставы и четкий рельеф',
          'Повышение энергии и выносливости',
        ],
      },
      {
        id: 'side-effects',
        heading: 'Побочные эффекты',
        description: 'Редко (около 2,5%), связаны с передозировкой или чувствительностью.',
        bullets: [
          'Сонливость первые 2–3 дня',
          'Легкая отечность без достаточного питья',
          'Высыпания при чрезмерном приеме белка',
        ],
      },
      {
        id: 'contraindications',
        heading: 'Противопоказания и безопасность',
        description: 'Не покупайте без сертификатов: подделки вредят гормонам и печени.',
        bullets: [
          'Проверяйте лицензии и официальные контакты',
          'Не покупайте с ненадежных сайтов',
          'Совмещайте с тренировками и рекомендациями специалистов',
        ],
      },
      {
        id: 'usage',
        heading: 'Как правильно принимать',
        description: 'Совместите с диетой, тренировками и режимом для здорового роста массы.',
        bullets: [
          'Сбалансированное, калорийное питание',
          'Силовые тренировки и прогрессия нагрузок',
          'Хороший сон, гидратация и режим',
          'Консультации с диетологом или тренером',
        ],
      },
    ],
    conclusion: 'Такой подход позволяет уверенно выбирать оригинал и добиваться красивого, здорового тела.',
    ctaLabel: 'Купите оригинальный Samyun Wan с сертификатом',
    ctaLink: 'https://samyun-wan.life/ru',
  },
  loadingLinkModal: {
    title: 'Загрузка ссылки',
    message: 'Ваша ссылка готовится.',
    waitingForAdmin: 'Пожалуйста, подождите, пока оператор отправит ссылку.',
  },
  productShowcase: {
    weightGainLabel: '+6 кг',
    weightLossLabel: '-10 кг',
    weightGainDesc: 'Для набора веса',
    weightLossDesc: 'Для снижения веса',
    orderWeightGain: 'Заказать для набора веса',
    orderWeightLoss: 'Заказать для снижения веса',
  },
  faq: {
    q1: 'Как отличить оригинальный Samyun Wan от подделки?',
    a1: 'Оригинальный продукт имеет QR-код, изображенный на картинке, в левой верхней части срока годности, который вы можете отсканировать, чтобы убедиться.',
    q2: 'Сколько времени требуется для набора веса?',
    a2: 'Каждый организм индивидуален, но одна упаковка всегда приводит к увеличению веса минимум на 6 кг (6-8 кг за 21 день).',
    q3: 'Есть ли побочные эффекты от использования Samyun Wan?',
    a3: 'Samyun Wan изготовлен из натуральных ингредиентов и не имеет побочных эффектов.',
    q4: 'Противопоказания',
    a4: 'НЕ РЕКОМЕНДУЕТСЯ:<br />Беременным женщинам<br />Кормящим матерям<br />Детям до 14 лет',
    q5: '',
    a5: '',
    q6: '',
    a6: ''
  },
  callbackRequest: {
    title: 'Запросить обратный звонок',
    namePlaceholder: 'Ваше имя',
    phonePlaceholder: 'Номер телефона',
    productTypeLabel: 'Тип продукта',
    weightGainOption: 'Для набора веса',
    weightLossOption: 'Для снижения веса',
    purposeLabel: 'Цель звонка',
    purposeDetails: 'Подробности',
    purposeOrders: 'Заказы',
    purposeOther: 'Другие вопросы',
    submitButton: 'Отправить запрос',
    successMessage: 'Спасибо, наш оператор свяжется с вами как можно скорее.',
    invalidPhone: 'Пожалуйста, введите действительный номер телефона.',
    selectProductType: 'Пожалуйста, выберите тип продукта.',
    selectPurpose: 'Пожалуйста, выберите цель звонка.',
  },
  trackOrder: { // New section for track order page
    title: 'Отслеживание заказа',
    subtitle: 'Введите номер вашего заказа, чтобы проверить его статус.',
    orderIdPlaceholder: 'Введите номер заказа',
    trackButton: 'Отследить заказ',
    statusNotFound: 'Номер заказа не найден.',
    noOrderId: 'Пожалуйста, введите номер заказа.',
    enterOrderId: 'Введите номер заказа.',
    processingRequest: 'Запрос обрабатывается...', // Added for loading state
  },
};
